...
1. **초기 문제점**  
 Bilateral Filter에만 의존할 경우 색상의 경계가 뭉개져 만화 특유의 느낌이 나지 않았습니다. 또한 피사체 얼굴과 몸에 Noise을 다수 생성했었습니다.
2. **개선 과정**
...
3. **한계점**
    알고리즘을 개선했음에도 입력 이미지의 해상도나 조명 상태에 따라 `threshold` 값이나 K-Means의 `K` 값을 매번 직접 수정해줘야 좋은 결과를 얻을 수 있다는 점이 가장 큰 한계점이라고 생각합니다. 향후 이미지의 밝기 히스토그램을 분석하여 임계값을 자동으로 조절하는(Otsu's Method 등)방식을 도입한다면 이를 개선애 도움이 될 수 있을 것이라 생각합니다.